...
reorderCols <- function(x, group, sort = NULL) {
  if(length(group) != ncol(x))
    stop("'group' vector should equal 'ncol(x)'")
  x.split <- split(data.frame(t(x)), group)
  idx.split <- split(seq_len(ncol(x)), group)
  
  if(!is.null(sort)){
    stat <- lapply(x.split, apply, 1, sort, na.rm=TRUE)
    for(i in seq_len(length(idx.split))){idx.split[[i]] <- idx.split[[i]][order(stat[[i]])]}
  }
...
mvtsplot <- function(x, group = NULL, xtime = NULL,
  ...
  if(is.null(xtime)) {
    ...
    xlim <- range(xtime)
  if(!is.null(group) && !is.null(sort)) {
    group <- as.factor(group)
    x <- reorderCols(x, group, sort)
  }
  if(!margin && !is.null(sort) && is.null(group)) {
    stat <- apply(x, 2, sort, na.rm = TRUE)
    x <- x[, order(stat)]
  }
  if(margin && is.null(group)) {
